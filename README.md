...
실제로 스택을 띄우고 `admin`/`agent-svc`/`bob` 신원으로 직접 검증한 결과입니다.
접근제어/격리는 `bash examples/verify_access_control.sh`로 재현 가능합니다(**PASS 8 / WARN 1**).

| 요구사항 | 결과 | 근거 |
|----------|:---:|------|
| docker compose 스택 기동 (PostgreSQL + MinIO + basic-auth) | ✅ | 4개 서비스 정상, 아티팩트 MinIO 저장 |
| **실제 Google ADK + OpenAI 멀티에이전트 트레이싱** | ✅ | `examples/adk_agent.py` → 23-span 트레이스, **subagent 패턴**(orchestrator→research/math) + 실제 토큰/비용 기록 |
| 프롬프트 버전 관리 | ✅ | register/alias 동작 |
| **admin 전용 유저 발급** (username/password) | ✅ | 무인증 401·비관리자 403·admin 200 |
| **프로젝트(experiment)별 권한 부여·격리** | ✅ | agent-svc=EDIT(쓰기), bob=READ(읽기/쓰기 403), 미부여 프로젝트 메타데이터 403 |
| 팀/그룹 · 조직 생성 Admin API · 멤버 초대/SSO | ❌ | OSS 미지원 (유저 단위 권한만) |
| **프로젝트별 트레이스 열람 격리** | ⚠️ **누수** | `GET experiment`은 403이지만 `/api/3.0/mlflow/traces/search`는 권한 무시 → 권한 없는 프로젝트 트레이스가 노출됨 |

> **핵심 결론**: "admin이 사람별 로그인 발급 + 프로젝트별 권한 부여"는 동작하지만,
> **정작 트레이스 데이터에는 프로젝트별 격리가 적용되지 않습니다.** 트레이싱이 핵심이고
> 멀티팀 트레이스 격리가 필요하면 MLflow OSS basic-auth는 부적합합니다.
...
### 5. 접근제어·트레이스 격리 자동 검증
```bash
bash examples/verify_access_control.sh
```
프로젝트 생성/유저 발급/권한 부여를 셋업한 뒤, 신원별 allow/deny와 **트레이스 격리 누수**를
status code로 검증해 PASS/FAIL/WARN로 출력합니다(상단 [검증 결과 요약](#-검증-결과-요약-실측) 참고).
